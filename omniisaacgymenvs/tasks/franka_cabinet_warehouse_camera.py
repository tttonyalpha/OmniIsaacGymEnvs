...
from omniisaacgymenvs.robots.articulations.warehouse import Warehouse


from omniisaacgymenvs.robots.articulations.views.cabinet_view import CabinetView
from omniisaacgymenvs.robots.articulations.views.franka_view import FrankaView
from omniisaacgymenvs.robots.articulations.views.warehouse_view import WarehouseView
...
class FrankaCabinetWarehouseCameraTask(RLTask):
    ...
    def set_up_scene(self, scene) -> None:
        ...
        self._warehouses = WarehouseView(prim_paths_expr="/World/envs/.*/warehouse", name="warehouse_view")
    ...
    def initialize_views(self, scene):
        ...
        if scene.object_exists("warehouse_view"):
            scene.remove_object("warehouse_view", registry_only=True)
    ...
    def get_warehouse(self):
        warehouse = Warehouse(self.default_zero_env_path + "/warehouse", name="warehouse")
        self._sim_config.apply_articulation_settings(
            "warehouse", get_prim_at_path(warehouse.prim_path), self._sim_config.parse_actor_config("warehouse")
        )
    ...
    def reset_idx(self, env_ids):
        ...
        #reser warehouse
        self._warehouses.set_joint_positions(
            torch.zeros_like(self._warehouses.get_joint_positions(clone=False)[env_ids]), indices=indices
        )
        self._warehouses.set_joint_velocities(
            torch.zeros_like(self._warehouses.get_joint_velocities(clone=False)[env_ids]), indices=indices
        )
